            ...
            <div class="tasks__table">
                ...
                <div class="tasks__todo">
                    <table class="table table-striped task">
                        ...
                        <tr v-for="(todo,index) in todos" class="row">

                            <td scope="row" class="task__text col-6 col-lg-8  text-left" v-model="todo.text">{{todo.text}}</td>
                            <td v-if="startEdit==true" class="col-6 col-lg-8 position-absolute d-flex" colspan="">
                                <textarea class="new_text"  rows="1">{{todo.text}}</textarea>
                                <button type="button" class="btn btn-success" v-on:click="editItem(index)">Сохранить</button>
                            </td>
                            <td class="task__edit col-3 col-lg-2"><button type="button" class="btn btn-success" v-on:click="startEdit=true">Изменить</button></td>
